...
export function useUpdateBudgetItem(budgetId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ itemId, ...data }: Partial<BudgetItem> & { itemId: string }) => {
      const { data: updated } = await axios.patch(`/api/budgets/${budgetId}/items/${itemId}`, data)
      return updated
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["budget-items", budgetId] })
...
export function useDuplicateBudgetItem(budgetId: string) {
  ...
  return useMutation({
    mutationFn: async (itemId: string) => {
      ...
      const itemToDuplicate = items.find((item: BudgetItem) => item.id === itemId)
      
      if (!itemToDuplicate) throw new Error("Item não encontrado")
      
      const { id, budgetId: bId, createdAt, updatedAt, ...copyData } = itemToDuplicate
      const { data: newEntry } = await axios.post(`/api/budgets/${budgetId}/items`, {
        ...copyData,
...
export function useUpdateBudgetRates(budgetId: string) {
  ...
  return useMutation({
    mutationFn: async (rates: BudgetRates) => {
      const { data } = await axios.patch(`/api/budgets/${budgetId}/rates`, rates)
      return data
    },
    onSuccess: () => {
...
export function useImageTypes() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return data as Array<{ id: string; name: string; baseHours: number; icon?: string }>
    },
  })
}
...
export function useDifficulties() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return data as Array<{ id: string; name: string; multiplier: number; criteria?: string }>
    },
  })
}
...
export function useOperationalCosts() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const fixedTotal = (fixedRes.data as any[]).reduce((s, c) => s + c.value, 0)
