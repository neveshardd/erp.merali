...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      newTotal: total,
      paymentTerms: "O valor deste aditivo será faturado conforme cronograma negociado, mantendo a política de 50% de entrada e 50% na entrega final.",
      justification: "Inclusão de novos itens e ajustes de escopo solicitados pelo cliente para expansão das visualizações do projeto."
    })
  } catch (error) {
    console.error("[ADDENDUM_GET]", error)
    return NextResponse.json({ error: "Erro ao carregar Aditivo de Escopo" }, { status: 500 })
  }
}
