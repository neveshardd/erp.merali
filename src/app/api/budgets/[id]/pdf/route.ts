...
export async function GET(
  ...
  try {
    ...
    const dateFormatted = format(budget.createdAt, "dd 'de' MMMM, yyyy", { locale: ptBR })

    // Validity: 7 days from creation
    const validUntil = new Date(budget.createdAt)
    validUntil.setDate(validUntil.getDate() + 7)
    const validUntilFormatted = format(validUntil, "dd 'de' MMMM, yyyy", { locale: ptBR })
    ...
    return NextResponse.json({ error: "Erro ao gerar dados do PDF" }, { status: 500 })
  }
}
