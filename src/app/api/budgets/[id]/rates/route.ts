...
export async function GET(
  ...
  try {
    ...
    const budget = await prisma.budget.findUnique({
      where: { id },
      select: { baseHours: true, hourlyRate: true, profitMargin: true, multiplier: true },
    })
...
export async function PATCH(
  ...
  try {
    ...
    const { baseHours, hourlyRate, profitMargin, multiplier } = ratesSchema.parse(body)
    ...
    for (const item of items) {
      ...
      const unitValue = item.baseHoursUnit * item.difficultyMultiplier * hourlyRate * multiplier * (1 + profitMargin / 100)
    ...
    await prisma.budget.update({ where: { id }, data: { totalValue: parseFloat(total.toFixed(2)) } })
    ...
    return NextResponse.json({ error: "Erro ao atualizar taxas" }, { status: 500 })
  }
}
