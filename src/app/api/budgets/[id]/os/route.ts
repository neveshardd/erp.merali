...
export async function GET(
  ...
  try {
    ...
    const totalHours = budget.items.reduce((s, i) => s + i.estimatedHours * i.quantity, 0)
    ...
    return NextResponse.json({
      ...
      items: budget.items.map((item, idx) => ({
        ...
        notes: "Focar em referências de materiais e iluminação realista conforme briefing.",
      })),
    ...
    return NextResponse.json({ error: "Erro ao carregar Ordem de Serviço" }, { status: 500 })
  }
}
