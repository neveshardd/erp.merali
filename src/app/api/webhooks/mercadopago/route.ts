...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `[MercadoPago Webhook] Received ${action || "notification"} for ${type || "resource"}:`,
      body,
    );
    ...
    if (type === "payment" || action?.startsWith("payment.")) {
      ...
      const invoiceId =
        paymentData.metadata?.invoice_id || paymentData.external_reference;

      if (!invoiceId) {
        console.warn(
          `[MercadoPago Webhook] No invoiceId associated with payment ${paymentId}`,
        );
      ...
      switch (paymentData.status) {
        case "approved": {
          ...
          console.log(
            `✅ Invoice ${invoiceId} marked as PAID via Mercado Pago payment ${paymentId}`,
          );
        ...
        case "cancelled": {
          ...
          console.warn(
            `❌ Mercado Pago payment ${paymentId} was ${paymentData.status} for Invoice ${invoiceId}`,
          );
        ...
        case "in_process":
          console.log(
            `⏳ Mercado Pago payment ${paymentId} is still pending for Invoice ${invoiceId}`,
          );
          break;
