...
import { useDifficulties, useDeleteDifficulty } from "@/hooks/use-settings"
    ...
    return (
        <main className="flex flex-col gap-8 p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tighter text-neutral-900 dark:text-neutral-100 uppercase">Graus de Dificuldade</h1>
                    <p className="text-neutral-500 text-sm font-bold uppercase tracking-widest mt-1">Níveis de complexidade técnica e seus multiplicadores</p>
                </div>
                <Button 
                    onClick={() => {
            ...
            <div className="flex flex-col gap-6">
                {isLoading ? (
                    ...
                    filteredDifficulties.map((diff: any, i: number) => {
                        ...
                        return (
                            <Card key={diff.id} className="border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 rounded-3xl overflow-hidden shadow-sm group hover:border-neutral-900 dark:hover:border-white transition-all">
                                <CardContent className="p-0">
                                    <div className="flex items-center">
                                        <div className={`w-24 h-24 sm:w-32 sm:h-32 ${style.bg} ${style.color} flex items-center justify-center border-r border-neutral-100 dark:border-neutral-800 shrink-0`}>
                                        ...
                                        <div className="p-6 sm:p-8 flex flex-1 justify-between items-center gap-6">
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center gap-3">
                                                    <h3 className="text-lg font-black uppercase text-neutral-900 dark:text-white leading-none">{diff.name}</h3>
                                                    <Badge className={`text-[9px] font-black uppercase tracking-widest ${style.bg} ${style.color} border-none shadow-none`}>
                                            ...
                                            <div className="hidden sm:flex items-center gap-4">
                                                <div className="flex flex-col items-end">
                                                     <span className="text-[10px] font-black text-neutral-300 uppercase tracking-widest mb-1">Impacto Financeiro</span>
                                                     <span className={`text-2xl font-black tabular-nums ${multiplier > 1 ? 'text-orange-500' : multiplier < 1 ? 'text-emerald-500' : 'text-blue-500'}`}>
