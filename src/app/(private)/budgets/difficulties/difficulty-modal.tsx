...
import { Gauge, Save, TrendingUp, Loader2 } from "lucide-react"
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { difficultySchema, DifficultyInput } from "@/schemas/settings"
...
export function DifficultyModal({ open, onOpenChange, difficulty }: DifficultyModalProps) {
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-xl font-black uppercase tracking-tighter">
                                    {difficulty ? "Editar Grau de Dificuldade" : "Novo Grau de Dificuldade"}
                                </DialogTitle>
                                <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-1">Configurar complexidade e impacto</p>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="p-8 flex flex-col gap-6 bg-white dark:bg-neutral-950">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Nome do Nível</Label>
                            <Input 
                                {...register("name")}
                            ...
                            />
                            {errors.name && (
                                <p className="text-red-500 text-[10px] uppercase font-bold">{errors.name.message}</p>
                            )}
                        </div>

                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Multiplicador de Complexidade</Label>
                            ...
                            {errors.multiplier && (
                                <p className="text-red-500 text-[10px] uppercase font-bold">{errors.multiplier.message}</p>
                            )}
                            <p className="text-[9px] font-bold text-neutral-400 uppercase">Impacto direto no cálculo de horas totais por imagem.</p>
                        </div>

                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Critérios de Avaliação</Label>
                            <Textarea 
                                {...register("criteria")}
