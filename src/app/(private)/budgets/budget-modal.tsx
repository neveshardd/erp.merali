...
import { FilePlus, Save, Layout, User, Layers, Loader2 } from "lucide-react"
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { budgetSchema, BudgetInput } from "@/schemas/budget"
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-1">
                                    {budget ? "Atualize os dados básicos do projeto" : "Inicie um novo projeto Merali Studio"}
                                </p>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="p-8 flex flex-col gap-6 bg-white dark:bg-neutral-950">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Nome do Projeto</Label>
                            ...
                            {errors.projectName && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.projectName.message}</p>}
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Selecionar Cliente</Label>
                                <Select 
                                ...
                                {errors.clientId && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.clientId.message}</p>}
                            </div>
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Perfil do Cliente</Label>
                                <Select 
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {clientTypes.map((type: any) => (
                                            <SelectItem key={type.id} value={type.name} className="font-bold uppercase text-[10px]">
                                                {type.name} ({type.multiplier.toFixed(2)}x)
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                                {errors.clientTypeName && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.clientTypeName.message}</p>}
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Categoria do Projeto</Label>
                                <Select 
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectItem value="Residencial" className="font-bold uppercase text-[10px]">Residencial</SelectItem>
                                        <SelectItem value="Comercial" className="font-bold uppercase text-[10px]">Comercial</SelectItem>
                                        <SelectItem value="Urbanismo" className="font-bold uppercase text-[10px]">Urbanismo</SelectItem>
                                        <SelectItem value="Interiores" className="font-bold uppercase text-[10px]">Interiores</SelectItem>
                                        <SelectItem value="Design de Objeto" className="font-bold uppercase text-[10px]">Design de Objeto</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Prazo Estimado (Ex: 4 semanas)</Label>
                                <Input 
                                    {...register("deadline")}
                        ...
                        {!budget && (
                            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest leading-relaxed">
                                    Ao criar o orçamento, você será direcionado para a tela de composição de imagens, onde poderá definir quantidades e dificuldades para o cálculo automático de horas.
                                </p>
                            </div>
                        )}
                    </div>
