...
import { useDeleteVariableCost, useVariableCosts } from "@/hooks/use-costs";
...
export default function VariableCostsPage() {
  ...
  const filteredCosts = variableCosts.filter(
    (c: any) =>
      (c.description || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (c.budget?.projectName || "")
        .toLowerCase()
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm font-bold uppercase tracking-widest mt-1">
            Despesas pontuais vinculadas a projetos específicos
          </p>
        </div>
        <Button
          ...
          <CardContent className="p-8 flex flex-col gap-2 relative">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-black tabular-nums tracking-tighter text-neutral-900 dark:text-white">
                {String(
                  new Set(
                    variableCosts.map((c: any) => c.budgetId).filter(Boolean),
                  ).size,
                ).padStart(2, "0")}
              </span>
