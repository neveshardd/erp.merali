...
import { useDeleteFixedCost, useFixedCosts } from "@/hooks/use-costs";
...
export default function FixedCostsPage() {
  ...
  const filteredCosts = fixedCosts.filter(
    (c: any) =>
      (c.description || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (c.category || "").toLowerCase().includes(searchQuery.toLowerCase()),
  );

  const totalCost = fixedCosts.reduce(
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm font-bold uppercase tracking-widest mt-1">
            Gerenciamento de despesas operacionais do estúdio
          </p>
        </div>
        <Button
        ...
        <Card className="border border-neutral-200 dark:border-neutral-800 bg-emerald-50/30 dark:bg-emerald-900/10 rounded-3xl shadow-sm border-dashed">
          <CardContent className="p-8 flex flex-col gap-2">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[10px] font-bold text-emerald-600/40 uppercase tracking-widest">
                Base de Preço
              </span>
            </div>
          </CardContent>
      ...
      <div className="p-8 bg-blue-50 dark:bg-blue-900/10 rounded-3xl border border-blue-100 dark:border-blue-900/30 flex items-start gap-4">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">
            Por que gerenciar custos fixos?
          </span>
          <p className="text-xs font-bold text-blue-900/60 uppercase leading-relaxed tracking-tight">
            Seus custos fixos determinam o valor mínimo da sua hora de trabalho.
            Manter estes dados atualizados garante que seus orçamentos cubram
            não apenas a produção, mas toda a infraestrutura vital do Merali
            Studio.
          </p>
        </div>
      </div>
