...
export function AddItemModal({ open, onOpenChange, budgetId, item }: AddItemModalProps) {
    const { data: imageTypes = [], isLoading: loadingTypes } = useImageTypes()
    const { data: difficulties = [], isLoading: loadingDiffs } = useDifficulties()
    ...
    const unitValue = estimatedHours * hourlyRate * multiplier * (1 + profitMargin / 100)
    const totalValue = unitValue * quantity

    const handleSave = async () => {
        if (!description.trim()) { toast.error("Informe a identificação da imagem."); return }
        if (!imageTypeName) { toast.error("Selecione o tipo de imagem."); return }
        if (!difficultyName) { toast.error("Selecione a dificuldade."); return }
    ...
    const selectTriggerClass = "h-11 border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 px-4 font-bold text-neutral-900 dark:text-neutral-100 focus:ring-neutral-900 dark:focus:ring-white rounded-lg"
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[540px] p-0 border-none shadow-2xl rounded-2xl overflow-hidden bg-white dark:bg-neutral-900">
                ...
                <div className="p-8 flex flex-col gap-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            {loadingTypes ? (
                                <div className="h-11 flex items-center px-4"><Loader2 className="w-4 h-4 animate-spin text-neutral-400" /></div>
                            ) : (
                                <Select value={imageTypeName} onValueChange={setImageTypeName}>
                                    ...
                                    <SelectContent className="border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-xl rounded-xl">
                                        {imageTypes.map(t => (
                                            <SelectItem key={t.id} value={t.name} className="font-bold text-xs uppercase py-3">
                                                {t.name} <span className="text-neutral-400 normal-case font-medium">({t.baseHours}h)</span>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                        ...
                        <div className="grid gap-2">
                            ...
                            {loadingDiffs ? (
                                ...
                                <Select value={difficultyName} onValueChange={setDifficultyName}>
                                    <SelectTrigger className={selectTriggerClass}>
                                        <SelectValue placeholder="Selecione..." />
                                    </SelectTrigger>
                                    <SelectContent className="border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-xl rounded-xl">
                                        {difficulties.map(d => (
                                            <SelectItem key={d.id} value={d.name} className="font-bold text-xs uppercase py-3">
                                                {d.name} <span className="text-neutral-400 normal-case font-medium">({d.multiplier}x)</span>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                    ...
                    {imageTypeName && difficultyName && (
                        <div className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 flex flex-col gap-3">
                            <span className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Cálculo do Item</span>
                            <div className="grid grid-cols-3 gap-4">
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400">Horas Estimadas</span>
                                    <span className="text-lg font-black text-neutral-900 dark:text-white tabular-nums">{estimatedHours.toFixed(1)}h</span>
                                </div>
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400">Unitário</span>
                                    <span className="text-lg font-black text-neutral-900 dark:text-white tabular-nums">{formatCurrency(unitValue)}</span>
                                </div>
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400">Total</span>
                                    <span className="text-lg font-black text-emerald-600 dark:text-emerald-400 tabular-nums">{formatCurrency(totalValue)}</span>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <DialogFooter className="p-6 bg-neutral-50/50 dark:bg-neutral-800/20 border-t border-neutral-100 dark:border-neutral-800 flex sm:justify-end gap-3">
                    ...
                    >
                        {(createItem.isPending || updateItem.isPending) ? <><Loader2 className="w-4 h-4 animate-spin" />Salvando...</> : (item ? "Atualizar Item" : "Adicionar Item")}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
