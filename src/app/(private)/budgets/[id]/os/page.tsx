...
export default function ServiceOrderPage() {
    ...
    const [checkedSteps, setCheckedSteps] = useState<Record<string, Record<string, boolean>>>({})
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-neutral-100 dark:bg-neutral-950">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-neutral-500 font-bold uppercase tracking-widest text-[10px]">Gerando Ordem de Serviço...</p>
                </div>
            </div>
        )
    }
    ...
    if (!os) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <p className="text-neutral-500 font-bold uppercase tracking-widest text-[10px]">Ordem de Serviço não encontrada.</p>
            </div>
        )
    }
            ...
            <div className="max-w-[21cm] mx-auto mb-8 flex justify-between items-center print:hidden">
                ...
                <div className="flex gap-3">
                    ...
                    <PDFDownloadLink
                        ...
                        document={<ServiceOrderPDFDocument data={os} checkedSteps={checkedSteps} checkedQI={checkedQI} />}
            ...
            <div className="max-w-[21cm] mx-auto bg-white shadow-2xl print:shadow-none min-h-[29.7cm] p-[1.5cm] flex flex-col gap-8 text-neutral-900 relative">
                ...
                <header className="flex justify-between items-center border-b-4 border-neutral-900 pb-6">
                    <div className="flex flex-col gap-2">
                        ...
                        <h1 className="text-2xl font-black uppercase tracking-tighter mt-2">Ordem de Serviço</h1>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                        ...
                        <span className="text-sm font-black mt-2 tracking-tighter">{os.number}</span>
                        <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Emissão: {os.date}</span>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-3 gap-6">
                    <div className="p-4 bg-neutral-50 border border-neutral-100 rounded-xl flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-neutral-400">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest">Projeto</span>
                        </div>
                        <span className="text-xs font-black uppercase leading-tight truncate">{os.project}</span>
                    </div>
                    <div className="p-4 bg-neutral-50 border border-neutral-100 rounded-xl flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-neutral-400">
                        ...
                        <span className="text-xs font-black uppercase leading-tight truncate">{os.client}</span>
                    </div>
                    <div className="p-4 bg-neutral-50 border border-neutral-100 rounded-xl flex flex-col gap-2 border-l-4 border-l-red-500">
                        <div className="flex items-center gap-2 text-neutral-400">
                            <Clock className="w-3 h-3" />
                            <span className="text-[9px] font-black uppercase tracking-widest">Prazo Final</span>
                        </div>
                        <span className="text-xs font-black uppercase leading-tight text-red-600">{os.deadline}</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    <h2 className="text-[10px] font-black uppercase tracking-widest text-neutral-400 border-b border-neutral-100 pb-2 flex items-center gap-2">
                        <Layout className="w-3 h-3" /> Itens de Produção & Briefing ({os.items.length})
                    </h2>
                    <div className="flex flex-col gap-4">
                        {os.items.map((item: any, i: number) => (
                            <div key={i} className="border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                                <div className="bg-neutral-900 text-white p-3 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] font-black bg-white/20 px-2 py-0.5 rounded">#{item.id}</span>
                                        <span className="text-xs font-black uppercase tracking-tight">{item.description}</span>
                                    </div>
                                    <span className="text-[9px] font-bold uppercase tracking-widest opacity-60 bg-white/10 px-2 py-1 rounded">{item.type} | {item.complexity}</span>
                                </div>
                                <div className="p-4 flex flex-col gap-3">
                                    <div className="flex items-start gap-4">
                                        <div className="flex flex-col gap-1 min-w-[120px] p-2 bg-neutral-50 rounded-lg">
                                            <span className="text-[8px] font-black uppercase text-neutral-400 tracking-widest">Tempo Estimado</span>
                                            <span className="text-sm font-black tabular-nums">{Number(item.hours).toFixed(1)}h {item.quantity > 1 ? `(x${item.quantity})` : ''}</span>
                                        </div>
                                        <div className="flex flex-col gap-1 flex-1">
                                            <span className="text-[8px] font-black uppercase text-neutral-400 tracking-widest">Notas Técnicas / Requisitos</span>
                                            <p className="text-[10px] font-medium text-neutral-600 uppercase leading-relaxed text-justify">{item.notes}</p>
                                        </div>
                                    </div>
                                    <div className="mt-2 pt-3 border-t border-neutral-100 grid grid-cols-4 gap-4">
                                        {['Modelagem', 'Materiais', 'Iluminação', 'Pós'].map(p => (
                                            ...
                                            >
                                                <div className={`w-4 h-4 border-2 rounded shrink-0 transition-colors flex items-center justify-center ${
                                                    checkedSteps[item.id]?.[p] 
                                                        ? 'bg-neutral-900 border-neutral-900' 
                                                        : 'border-neutral-200'
                                                }`}>
                                                    {checkedSteps[item.id]?.[p] && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                                </div>
                                                <span className={`text-[8px] font-black uppercase tracking-tighter ${
                                                    checkedSteps[item.id]?.[p] ? 'text-neutral-900' : 'text-neutral-400'
                                                }`}>{p}</span>
                                            </button>
                                        ))}
                                    </div>
                            ...
                            {[
                                ...
                                >
                                    <div className={`w-4 h-4 border-2 rounded shrink-0 transition-colors flex items-center justify-center ${
                                        ...
                                        {checkedQI[i] && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-4 p-4 bg-neutral-50 border border-neutral-100 rounded-xl">
                            <div className="flex justify-between items-center text-[10px] font-bold uppercase">
                                <span className="text-neutral-400 tracking-widest">Responsável</span>
                                <span className="text-neutral-900">{os.manager}</span>
                            </div>
                            <div className="w-full h-px bg-neutral-200"></div>
                            <div className="flex justify-between items-center text-[10px] font-bold uppercase">
                                <span className="text-neutral-400 tracking-widest">Capacidade Total</span>
                                <span className="text-xl font-black tabular-nums tracking-tighter">{Number(os.totalHours).toFixed(1)}h de produção</span>
                            </div>
                        </div>
                        <div className="mt-4 flex flex-col gap-4">
                            <div className="w-full h-12 border border-dashed border-neutral-300 rounded flex items-center justify-center bg-neutral-50/30">
                                <span className="text-[8px] font-bold text-neutral-300 uppercase tracking-widest">Visto da Coordenação</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <footer className="mt-12 pt-8 border-t border-neutral-100 flex justify-between items-center">
                    <div className="flex flex-col gap-1">
                        <span className="text-[8px] font-black text-neutral-400 uppercase tracking-[0.2em]">Merali Studio | Internal Pipeline v2.0</span>
                        <span className="text-[7px] text-neutral-300 uppercase font-medium">Este documento é de uso interno. Informações confidenciais.</span>
                    </div>
                    <div className="flex gap-4">
                        <div className="flex items-center gap-1 group">
                            <div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></div>
                            <span className="text-[8px] font-black text-red-600 uppercase tracking-widest">Alta Prioridade</span>
                        </div>
                        <span className="text-[8px] font-bold text-neutral-300 uppercase tracking-widest">Documento ID: {id.slice(0,8).toUpperCase()}</span>
                    </div>
                </footer>
