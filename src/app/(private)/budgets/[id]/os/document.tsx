...
export function ServiceOrderPDFDocument({ data, checkedSteps = {}, checkedQI = {} }: OSPDFProps) {
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                <View style={styles.header}>
                    <View>
                        ...
                        <Text style={[styles.infoValue, { marginTop: 10, fontSize: 10 }]}>Ordem de Serviço</Text>
                    </View>
                ...
                <View style={styles.infoGrid}>
                    ...
                    <View style={[styles.infoCol, { borderLeftWidth: 4, borderLeftColor: '#DC2626' }]}>
                        <Text style={styles.infoLabel}>Prazo Final</Text>
                        <Text style={[styles.infoValue, { color: '#DC2626' }]}>{data.deadline}</Text>
                    </View>
                </View>

                <View>
                    <Text style={styles.sectionTitle}>Itens de Produção & Briefing ({data.items.length})</Text>
                    {data.items.map((item, i) => (
                        <View key={i} style={styles.itemCard}>
                            <View style={styles.itemHeader}>
                                <Text style={styles.itemTitle}>#{item.id} {item.description}</Text>
                                <Text style={styles.itemBadge}>{item.type} | {item.complexity}</Text>
                            </View>
                            <View style={styles.itemBody}>
                                <View style={styles.itemStat}>
                                    <Text style={styles.statLabel}>Tempo Estimado</Text>
                                    <Text style={styles.statValue}>{Number(item.hours).toFixed(1)}h {item.quantity > 1 ? `(x${item.quantity})` : ''}</Text>
                                </View>
                            ...
                            <View style={[styles.checklistRow, { paddingHorizontal: 10, paddingBottom: 8 }]}>
                                {['Modelagem', 'Materiais', 'Iluminação', 'Pós'].map(p => {
                                    ...
                                    return (
                                        <View key={p} style={styles.checkItem}>
                                            <View style={[
                                                styles.checkSquare, 
                                                isChecked ? { backgroundColor: '#171717', borderColor: '#171717' } : {}
                                            ]} />
                                            <Text style={[
                                                styles.checkLabel,
                ...
                <View wrap={false} style={styles.bottomGrid}>
                    ...
                    <View style={styles.bottomSection}>
                        ...
                        <View style={styles.managementBox}>
                            ...
                            <View style={[styles.managementRow, { borderTopWidth: 1, borderTopColor: '#EEEEEE', paddingTop: 6 }]}>
                                <Text style={styles.managementLabel}>Capacidade Total</Text>
                                <Text style={[styles.managementValue, { fontSize: 10 }]}>{Number(data.totalHours).toFixed(1)}h de produção</Text>
                            </View>
                        </View>
                ...
                <View style={styles.footer}>
                    <View>
                        <Text style={styles.footerText}>Merali Studio | Internal Pipeline v2.0</Text>
                        <Text style={[styles.footerText, { fontSize: 6, color: '#E5E5E5' }]}>Uso Interno - Confidencial</Text>
                    </View>
