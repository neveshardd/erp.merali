...
import { useCreateCheckout, useDeleteInvoice } from "@/hooks/use-invoices";
...
export default function BudgetDetailsPage() {
  ...
  const [confirmDeleteInvoiceOpen, setConfirmDeleteInvoiceOpen] =
    useState(false);
  const [pendingDeleteId, setPendingDeleteId] = useState<string | null>(null);
  const [paymentModalOpen, setPaymentModalOpen] = useState(false);
  const [pendingCheckoutInvoiceId, setPendingCheckoutInvoiceId] = useState<
    string | null
  >(null);
  ...
  const documentActions = [
    ...
    },
    {
      icon: FileCheck,
      label: "Contrato",
      sub: "GERAR CONTRATO PADRÃO",
      color: "text-emerald-600",
      bg: "bg-emerald-50 dark:bg-emerald-900/10",
      border: "border-emerald-100 dark:border-emerald-900/30",
      href: `/budgets/${id}/contract`,
    },
    {
      icon: Receipt,
      label: "Recibo",
      sub: "COMPROVANTE DE PAGAMENTO",
      color: "text-orange-600",
      bg: "bg-orange-50 dark:bg-orange-900/10",
      border: "border-orange-100 dark:border-orange-900/30",
      href: `/budgets/${id}/receipt`,
    },
    {
      icon: ClipboardList,
      label: "Ordem de Serviço",
      sub: "DADOS PARA PRODUÇÃO",
    ...
    {
      icon: FilePlus,
      label: "Aditivo de Escopo",
      sub: "NOVOS ITENS NO PROJETO",
      color: "text-emerald-600",
      bg: "bg-emerald-50 dark:bg-emerald-900/10",
      border: "border-emerald-100 dark:border-emerald-900/30",
      href: `/budgets/${id}/addendum`,
    },
    {
      icon: CheckCircle2,
      label: "Aprovação Final",
      sub: "TERMO DE ENCERRAMENTO",
      color: "text-cyan-600",
      bg: "bg-cyan-50 dark:bg-cyan-900/10",
      border: "border-cyan-100 dark:border-cyan-900/30",
      href: `/budgets/${id}/approval`,
    },
    {
      icon: Camera,
      label: "Uso de Imagem",
      sub: "AUTORIZAÇÃO DE PORTFÓLIO",
      color: "text-purple-600",
      bg: "bg-purple-50 dark:bg-purple-900/10",
      border: "border-purple-100 dark:border-purple-900/30",
      href: `/budgets/${id}/usage`,
    },
    {
      icon: Info,
      label: "Briefing Técnico",
      sub: "ENVIAR LINK PARA O CLIENTE",
      color: "text-rose-600",
  ...
  const rates = {
    baseHours: budget.baseHours,
    hourlyRate: budget.hourlyRate,
    profitMargin: budget.profitMargin,
    multiplier: budget.multiplier,
  };
  const _effectiveRate =
    rates.hourlyRate * rates.multiplier * (1 + rates.profitMargin / 100);
  ...
  return (
    <main className="flex flex-col gap-8 p-6 pb-20">
      ...
      <div className="flex flex-col gap-4">
        <Link
          href="/budgets"
          className="flex items-center gap-2 text-sm text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors w-fit font-bold uppercase tracking-widest text-[10px]"
        >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-3">
              <Badge
                variant="outline"
                className="bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 border-none font-bold uppercase tracking-wider text-[10px] px-2 py-0.5"
              >
              ...
              <span className="text-sm text-neutral-500 font-bold uppercase tracking-tight text-[11px]">
                ...
                {budget.client?.company && (
                  <span className="ml-2 text-neutral-400 font-bold">
                    ({budget.client.company})
                  </span>
                )}
              </span>
        ...
        <CardContent className="p-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-8 flex-1">
              {[
                ...
                <div key={i} className="flex flex-col gap-1">
                  ...
                  <span className="text-lg font-black text-neutral-900 dark:text-neutral-100 tabular-nums uppercase">
                    {m.value}
                  </span>
                </div>
              ))}
            ...
            <div className="flex flex-col gap-1 items-start md:items-end md:pl-8 md:border-l border-neutral-100 dark:border-neutral-800 shrink-0">
              <span className="text-[9px] uppercase font-black text-neutral-900 dark:text-white tracking-widest leading-none opacity-40">
                Preço Final
              </span>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {documentActions.map((action, i) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-lg ${action.bg} flex items-center justify-center ${action.color} border ${action.border} group-hover:scale-105 transition-transform`}
              >
                <action.icon className="w-6 h-6" />
              </div>
              <div className="flex flex-col">
                <span className="text-xs font-bold text-neutral-900 dark:text-neutral-100 uppercase tracking-tight">
                  {action.label}
                </span>
                <span className="text-[9px] font-bold text-neutral-400 group-hover:text-neutral-900 dark:group-hover:text-white transition-colors uppercase tracking-widest">
                  {action.sub}
                </span>
              </div>
            </Link>
          ...
          <Table>
            <TableHeader className="bg-neutral-50/50 dark:bg-neutral-800/50">
              <TableRow className="hover:bg-transparent border-b border-neutral-100 dark:border-neutral-800">
                <TableHead className="font-black text-[10px] uppercase tracking-widest py-4 px-8">
                  Imagem / Descrição
                </TableHead>
                ...
                <TableHead className="text-right font-black text-[10px] uppercase tracking-widest px-8">
                  Ações
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {items.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={7}
                    className="h-40 text-center text-neutral-400 font-bold uppercase tracking-widest text-[10px]"
                  >
                ...
                items.map((item) => (
                  <TableRow
                    key={item.id}
                    className="group hover:bg-neutral-50 dark:hover:bg-neutral-800/30 transition-colors border-b border-neutral-100 dark:border-neutral-800"
                  >
                    ...
                    <TableCell className="py-4">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">
                          {item.difficultyName} ({item.difficultyMultiplier}x)
                        </span>
                      </div>
                    </TableCell>
          ...
          <div className="p-8 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/30 dark:bg-neutral-800/10">
            <div className="flex justify-end gap-8 items-center">
              ...
              <div className="flex flex-col items-end gap-0.5">
                ...
                <span className="text-3xl font-black text-emerald-500 dark:text-emerald-400 tabular-nums">
                  {formatCurrency(budget.totalValue ?? 0)}
                </span>
              </div>
            </div>
      ...
      <Card className="border border-neutral-200 dark:border-neutral-800 shadow-none bg-white dark:bg-neutral-900 rounded-3xl overflow-hidden">
            ...
            <TableBody>
              {budget.invoices?.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={5}
                    className="h-32 text-center text-neutral-400 font-bold uppercase tracking-widest text-[10px]"
                  >
        ...
        {(budget.invoices?.length ?? 0) > 0 &&
          (() => {
            const totalInvoiced =
              budget.invoices?.reduce(
                (a: number, inv: any) => a + inv.amount,
                0,
              ) ?? 0;
            const totalPaid =
              budget.invoices
                ?.filter((inv: any) => inv.status === "PAID")
                .reduce((a: number, inv: any) => a + inv.amount, 0) ?? 0;
            const budgetTotal = budget.totalValue ?? 0;
            const pct =
              budgetTotal > 0
                ? Math.min((totalPaid / budgetTotal) * 100, 100)
                : 0;
            return (
              <div className="p-8 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/30 dark:bg-neutral-800/10 flex flex-col gap-4">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest text-neutral-400 text-right">
