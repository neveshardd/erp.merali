...
import { Image as ImageIcon, Save, Clock, Loader2, Box, Monitor, Layout, Camera } from "lucide-react"
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { imageTypeSchema, ImageTypeInput } from "@/schemas/settings"
...
export function ImageTypeModal({ open, onOpenChange, type }: ImageTypeModalProps) {
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <DialogHeader className="p-8 bg-purple-600 text-white">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-1">Configurar categoria e horas base</p>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="p-8 flex flex-col gap-6 bg-white dark:bg-neutral-950">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Nome da Categoria</Label>
                            <Input 
                                {...register("name")}
                        ...
                        <div className="space-y-3">
                            ...
                            {errors.baseHours && (
                                <p className="text-red-500 text-[10px] uppercase font-bold">{errors.baseHours.message}</p>
                            )}
                            <p className="text-[9px] font-bold text-neutral-400 uppercase">Tempo estimado para uma imagem padrão deste tipo.</p>
                        </div>

                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Ícone Representativo</Label>
                            <div className="grid grid-cols-4 gap-2">
                                {['Box', 'Monitor', 'Layout', 'Camera'].map((icon) => (
                                    ...
                                    >
                                        <span className="text-[8px] font-black uppercase">{icon}</span>
                                    </button>
                                ))}
                            </div>
