...
import { useDeleteImageType, useImageTypes } from "@/hooks/use-settings";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm font-bold uppercase tracking-widest mt-1">
            Estimativas de tempo base por categoria de render
          </p>
        </div>
        <Button
          onClick={() => {
        ...
        {isLoading ? (
          ...
          filteredTypes.slice(0, 5).map((type: any, i: number) => {
            ...
            return (
              <Card
                key={type.id}
                className="border-none bg-white dark:bg-neutral-900 shadow-sm rounded-2xl overflow-hidden text-center hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-all cursor-default"
              >
                <CardContent className="p-6 flex flex-col items-center gap-3">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[11px] font-black uppercase text-neutral-900 dark:text-white leading-tight">
                      {type.name}
                    </span>
                    <span className="text-[10px] font-bold text-neutral-400 tabular-nums uppercase">
                      {type.baseHours}H BASE
                    </span>
                  </div>
                </CardContent>
              </Card>
                  ...
                  >
                    <TableCell className="py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg ${style.bg} ${style.color} flex items-center justify-center`}
                        >
        ...
        {!isLoading && filteredTypes.length === 0 && (
          <div className="py-20 text-center text-neutral-500 flex flex-col items-center gap-2">
            ...
            <span className="font-bold uppercase tracking-widest text-[10px]">
              Nenhum tipo de imagem encontrado
            </span>
          </div>
        )}
      </Card>
