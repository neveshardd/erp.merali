...
function GalleryItem({ item }: { item: GroupItem }) {
  ...
  return (
    <View style={{ marginBottom: GAP }}>
      <Image src={item.url} style={{ width: "100%", borderRadius: 3 }} />
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.coverPage}>
        {coverImage && <Image src={coverImage.url} style={s.coverBg} />}
        <View style={s.coverOverlay} />
        <View style={s.coverContent}>
          <Text style={s.eyebrow}>
            {configs?.eyebrow || "High-End 3D Visualization Studio"}
          </Text>
          <Text style={s.coverTitle}>
            {configs?.coverTitle || "Merali\nStudio"}
          </Text>
          <Text style={s.coverSub}>
            {configs?.coverSubtitle ||
              "Visualização Arquitetônica de Alto Padrão"}
          </Text>
      ...
      <Page size="A4" style={s.manifestoPage}>
        ...
        <Text style={s.manifestoTitle}>
          {configs?.manifestoTitle ||
            "Transformamos visões em\nexperiências visuais."}
        </Text>
        <Text style={s.manifestoText}>
          {configs?.manifestoText ||
            "Na Merali Studio, acreditamos que a visualização 3D vai além de simples pixels. Nossa missão é capturar a intenção de cada projeto e traduzi-la em imagens que evocam emoção e clareza. Unimos precisão técnica com uma sensibilidade artística apurada, garantindo que cada render conte uma história e valorize cada detalhe da sua arquitetura."}
        </Text>
        <View
          style={[
            s.divider,
            { backgroundColor: WHITE, marginTop: 20, opacity: 0.2 },
          ]}
        />
      </Page>
      ...
      {pages.map((group, i) => (
        <GalleryPage
          ...
          footerText={
            configs?.footerText ||
            "Merali Studio de Visualização · meralistudio.com.br"
          }
        />
      ))}
      ...
      <Page size="A4" style={s.coverPage}>
        <View style={s.coverOverlay} />
        <View
          style={[s.coverContent, { alignItems: "center", paddingBottom: 80 }]}
        >
          <Text
            style={[
              s.eyebrow,
              { textAlign: "center", color: WHITE, opacity: 0.5 },
            ]}
          >
          ...
          <Text
            style={[
              s.coverSub,
              { textAlign: "center", marginBottom: 24, fontSize: 16 },
            ]}
          >
          ...
          <View style={{ alignItems: "center", gap: 8 }}>
            <Text style={[s.metaValue, { opacity: 0.6, fontSize: 10 }]}>
              {configs?.website || "meralistudio.com.br"}
            </Text>
            <Text style={[s.metaValue, { opacity: 0.6, fontSize: 10 }]}>
              {configs?.instagram || "@merali_studio"}
            </Text>
            <Text style={[s.metaValue, { opacity: 0.6, fontSize: 10 }]}>
              {configs?.contactEmail || "contato@meralistudio.com.br"}
            </Text>
          </View>
        </View>
      </Page>
