...
import { Search, Plus, Image as ImageIcon, Video, Trash2, Heart, Loader2, X, Maximize2 } from "lucide-react"
...
import { useMedia, useDeleteMedia, useUpdateMedia } from "@/hooks/use-media"
            ...
            <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button 
                    ...
                    className={cn(
                        ...
                        item.isFavorite ? "text-red-500 bg-white" : "text-neutral-600 hover:text-red-500"
                    )}
                    title={item.isFavorite ? "Remover dos favoritos" : "Adicionado aos favoritos"}
                >
            ...
            <div className="p-3 border-t border-neutral-100 dark:border-neutral-800">
                {editingId === item.id ? (
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        >
                            {item.name.includes('.') ? item.name.substring(0, item.name.lastIndexOf('.')) : item.name}
                        </p>
...
export default function MediaLibraryPage() {
    ...
    const saveEdit = async () => {
        if (!editingId) return
        try {
            const originalItem = mediaItems.find((i: MediaItem) => i.id === editingId)
            if (!originalItem) {
                setEditingId(null)
                return
            }
    ...
    return (
        <main className="flex flex-col gap-8 h-full p-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="font-bold text-3xl tracking-tight text-neutral-900 dark:text-neutral-100">Biblioteca de Mídia</h1>
                    <p className="text-neutral-500 mt-1">Gerencie suas fotos e vídeos do projeto</p>
                </div>
            ...
            <Dialog open={!!selectedMedia} onOpenChange={(open) => !open && setSelectedMedia(null)}>
                ...
                >
                    <DialogTitle className="sr-only">
                        Visualização de {selectedMedia?.type === "VIDEO" ? "Vídeo" : "Imagem"}
                    </DialogTitle>
            ...
            {isLoading ? (
                <div className="flex-1 flex flex-col items-center justify-center">
                    ...
                    <p className="text-neutral-500 mt-4 font-bold uppercase tracking-widest text-[10px]">Carregando biblioteca...</p>
                </div>
                ...
                <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl bg-neutral-50/50 dark:bg-neutral-900/20">
                    ...
                    <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100">Nenhum arquivo encontrado</h3>
                    <p className="text-neutral-500 mt-1 text-center max-w-sm">
                        {searchQuery ? "Tente buscar por outro termo." : "Comece adicionando imagens ou vídeos à sua biblioteca."}
                    </p>
                </div>
            ) : (
