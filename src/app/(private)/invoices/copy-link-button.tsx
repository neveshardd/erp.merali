...
import { Link2, QrCode, CreditCard, Landmark, Receipt, Check, Loader2 } from "lucide-react"
...
export function CopyLinkButton({ invoiceId }: CopyLinkButtonProps) {
    ...
    const handleCopy = async (method: PaymentMethod) => {
        setLoading(method)
        try {
            const { data } = await axios.post(`/api/invoices/${invoiceId}/checkout`, { method })
            if (data.url) {
                await navigator.clipboard.writeText(data.url)
                setCopied(method)
                toast.success(`Link de ${METHODS.find(m => m.key === method)?.label} copiado!`)
                ...
                <div className="flex flex-col gap-1">
                    {METHODS.map((m) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`w-8 h-8 rounded-lg ${m.bg} flex items-center justify-center ${m.color} shrink-0`}>
                                    {isLoading
                                ...
                                <span className={`ml-auto text-[9px] font-black uppercase tracking-wider shrink-0 transition-all ${isCopied ? 'text-emerald-500' : 'text-neutral-300 group-hover:text-neutral-500'}`}>
