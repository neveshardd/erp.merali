...
import { useInvoices, useDeleteInvoice, useCreateCheckout } from "@/hooks/use-invoices"
...
export default function InvoicesPage() {
    ...
    const executeCheckout = async (gateway: 'stripe' | 'mercadopago') => {
        ...
        try {
            const data = await createCheckout.mutateAsync({ id: pendingCheckoutId, gateway })
            if (data.url) {
    ...
    const filteredInvoices = invoices.filter((i: any) => 
        ...
        i.budget?.projectName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        i.id.toLowerCase().includes(searchQuery.toLowerCase())
    )

    const stats = {
        total: invoices.reduce((acc: number, curr: any) => acc + curr.amount, 0),
        pending: invoices.filter((i: any) => i.status === 'PENDING').reduce((acc: number, curr: any) => acc + curr.amount, 0),
        paid: invoices.filter((i: any) => i.status === 'PAID').reduce((acc: number, curr: any) => acc + curr.amount, 0),
    }
    ...
    return (
        <main className="flex flex-col gap-8 p-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tighter text-neutral-900 dark:text-neutral-100 uppercase">Faturamento & Cobrança</h1>
                    <p className="text-neutral-500 text-sm font-bold uppercase tracking-widest mt-1">Gestão de faturas e pagamentos via Mercado Pago</p>
                </div>
                ...
                <Card className="border-neutral-200 dark:border-neutral-800 shadow-none rounded-2xl bg-white dark:bg-neutral-900/50">
                    <CardContent className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Total Faturado</span>
                        </div>
                ...
                <Card className="border-neutral-200 dark:border-neutral-800 shadow-none rounded-2xl bg-white dark:bg-neutral-900/50">
                    <CardContent className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Pendente</span>
                        </div>
                ...
                <Card className="border-neutral-200 dark:border-neutral-800 shadow-none rounded-2xl bg-white dark:bg-neutral-900/50">
                    <CardContent className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Recebido</span>
                        </div>
            ...
            <Card className="border-neutral-200 dark:border-neutral-800 shadow-none rounded-2xl overflow-hidden">
                <Table>
                    <TableHeader className="bg-neutral-50 dark:bg-neutral-900/50">
                        <TableRow>
                            <TableHead className="font-bold uppercase tracking-widest text-[10px] h-14">ID / Projeto</TableHead>
                            <TableHead className="font-bold uppercase tracking-widest text-[10px] h-14">Cliente</TableHead>
                            <TableHead className="font-bold uppercase tracking-widest text-[10px] h-14">Vencimento</TableHead>
                    ...
                    <TableBody>
                        {filteredInvoices.map((i: any) => (
                            <TableRow key={i.id} className="group hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                                <TableCell className="font-black text-xs uppercase text-neutral-900 dark:text-white py-4">
                                    <div className="flex flex-col gap-0.5">
                                        <span className="text-[10px] text-neutral-400">#{i.id.slice(-6).toUpperCase()}</span>
                                        <span>{i.budget?.projectName || 'Serviços'}</span>
                                    </div>
                                </TableCell>
                                <TableCell className="text-xs font-bold text-neutral-500 uppercase">{i.client.name}</TableCell>
                                <TableCell className="py-4">
                                    <div className="flex items-center gap-2 text-xs font-bold text-neutral-500 uppercase">
                                        <Calendar className="w-3.5 h-3.5 opacity-40" />
                                        {format(new Date(i.dueDate), "dd MMM yyyy", { locale: ptBR })}
                                    </div>
                                </TableCell>
                                ...
                                <TableCell>
                                    <Badge className={`text-[9px] font-black uppercase tracking-widest rounded-full px-3 py-1 ${
                                        ...
                                        i.status === 'PENDING' ? "bg-amber-500/10 text-amber-600" :
                                        "bg-neutral-100 text-neutral-500"
                                    }`}>
                                        {i.status === 'PAID' ? 'Pago' : i.status === 'PENDING' ? 'Pendente' : i.status}
                                    </Badge>
                                </TableCell>
                                    ...
                                    <div className="flex justify-end gap-1">
                                        ...
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="ghost" size="icon" className="w-8 h-8 text-neutral-400 cursor-pointer">
