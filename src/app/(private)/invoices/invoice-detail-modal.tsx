  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl border-none shadow-2xl rounded-[32px] p-0 overflow-hidden bg-white dark:bg-neutral-900 leading-tight">
        {/* Visual Header */}
        <div
          className={`p-10 text-white ${isPaid ? "bg-emerald-500" : "bg-neutral-900"} relative overflow-hidden`}
        >
        ...
        <div className="p-8 space-y-10">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-8">
            ...
            <InfoBlock
              icon={FileText}
              label="Projeto"
              value={invoice.budget?.projectName || "Serviços"}
            />
            <InfoBlock
              icon={Calendar}
              label="Vencimento"
            ...
            <div className="pt-8 border-t border-neutral-100 dark:border-neutral-800">
              ...
              <div className="bg-neutral-50 dark:bg-neutral-800/40 rounded-[24px] p-6 space-y-4">
                <DetailRow
                  ...
                  value={invoice.paymentMethod?.split("_").join(" ") || "---"}
                />
                {invoice.paidAt && (
                  <DetailRow
                ...
                {details && (
                  <>
                    ...
                    {details.installments && (
                      <DetailRow
                        label="Parcelamento"
                        value={`${details.installments}x Sem Juros`}
                      />
                    )}
                    {details.card && (
                      <DetailRow
                        label="Cartão Utilizado"
                        value={`${details.card.brand} •••• ${details.card.last4 || details.card.last_four_digits}`}
                      />
                    )}
                    ...
                    <DetailRow
                      ...
                      value={invoice.stripeId || invoice.mercadopagoId}
                      isCode
                    />
                  </>
                )}
            ...
            <div className="p-6 bg-amber-50 dark:bg-amber-900/10 rounded-3xl border border-amber-100 dark:border-amber-900/20 flex gap-4 items-start">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[10px] font-bold text-amber-700/70 dark:text-amber-400/70 uppercase leading-relaxed max-w-[320px]">
                  O link de pagamento foi enviado ao cliente. Os detalhes da
                  transação aparecerão aqui assim que o pagamento for
                  confirmado.
                </p>
              </div>
            </div>
...
function InfoBlock({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <span
        className={`text-xs font-black uppercase tracking-tight truncate ${highlighted ? "text-emerald-600 dark:text-emerald-400 text-sm" : "text-neutral-900 dark:text-white"}`}
      >
        {value || "---"}
      </span>
    </div>
...
function DetailRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span
        className={`text-[10px] font-black uppercase text-neutral-900 dark:text-white tabular-nums ${isCode ? "bg-neutral-200/50 dark:bg-neutral-700/50 px-2 py-0.5 rounded text-[8px] font-mono" : ""}`}
      >
        {value || "---"}
      </span>
    </div>
