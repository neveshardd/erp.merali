...
export function DashboardPageContent() {
  ...
  const areaData =
    chartPoints.length > 0
      ? `${pathData} L ${chartPoints[chartPoints.length - 1].x} 200 L 0 200 Z`
      : "";
  ...
  return (
    <main className="flex flex-col gap-8 p-6">
      <TooltipProvider>
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold tracking-tighter text-neutral-900 dark:text-neutral-100 uppercase">
            Resumo Geral
          </h1>
          <p className="text-neutral-500 mt-1 uppercase text-[10px] font-bold tracking-widest leading-none">
            Bem-vindo ao Merali Studio
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <Card
              key={i}
              className="border border-neutral-200 dark:border-neutral-800 shadow-none bg-white dark:bg-neutral-900 rounded-xl overflow-hidden group hover:border-neutral-900 dark:hover:border-white transition-all cursor-pointer"
            >
              <CardHeader className="flex flex-row items-center justify-between pb-2 bg-neutral-50/50 dark:bg-neutral-800/30 px-5 pt-4">
                <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 leading-none">
                  {stat.title}
                </CardTitle>
              </CardHeader>
              <CardContent className="p-5 pt-4">
                <div className="text-2xl font-black text-neutral-900 dark:text-neutral-100 tabular-nums leading-none tracking-tighter">
                  {stat.value}
                </div>
                <div className="flex items-center gap-1.5 mt-3 text-[10px] font-bold uppercase tracking-widest text-neutral-500">
                  <stat.icon
                    className={`w-3 h-3 ${stat.color === "text-emerald-500" ? "text-emerald-500" : "text-neutral-400"}`}
                  />
                  <span
                    className={
          ...
          <Card className="lg:col-span-2 border border-neutral-200 dark:border-neutral-800 shadow-none bg-white dark:bg-neutral-900 rounded-xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b border-neutral-100 dark:border-neutral-800 pb-4 bg-neutral-50/50 dark:bg-neutral-800/30 px-6">
              <div>
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 mt-1">
                  Geração de receita por faturas pagas
                </CardDescription>
              </div>
              <span className="text-[10px] text-neutral-400 uppercase font-black tracking-widest opacity-30">
                BRL
              </span>
            </CardHeader>
              ...
              <div className="h-[320px] w-full relative flex flex-col justify-end">
                ...
                <div className="relative h-[256px] ml-16 mb-8 pb-4">
                  ...
                  >
                    ...
                    <path
                      d={areaData}
                      className="fill-neutral-900/5 dark:fill-white/5 transition-all duration-700"
                    />
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <div className="w-3 h-3 rounded-full bg-neutral-900 dark:bg-white border-2 border-white dark:border-neutral-900 cursor-pointer hover:scale-150 transition-transform shadow-sm" />
                            </TooltipTrigger>
                            <TooltipContent className="bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 border-none px-3 py-1.5 rounded-lg shadow-xl">
                ...
                {data?.lastBudgets.map((item, _i) => (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-black text-neutral-900 dark:text-neutral-100 uppercase tracking-tight group-hover:text-blue-600 transition-colors truncate max-w-[150px]">
                        {item.projectName}
                      </span>
                      <span className="text-[9px] uppercase tracking-widest text-neutral-400 font-bold truncate max-w-[150px]">
                        {item.client.name}
                      </span>
                    </div>
                    <Badge
                      variant="outline"
                      className="text-[9px] font-black bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700 px-2 py-0.5 text-neutral-500"
                    >
                      {item.status}
                    </Badge>
                  </Link>
