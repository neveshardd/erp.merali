...
import { ArrowLeft, Download, FileCheck, Loader2, Printer } from "lucide-react";
      ...
      <div className="max-w-[21cm] mx-auto bg-white shadow-2xl print:shadow-none min-h-[14cm] p-[2cm] flex flex-col gap-8 text-neutral-900 border-2 border-dashed border-neutral-100 print:border-none relative overflow-hidden">
        ...
        <div className="flex flex-col gap-6 py-8 relative z-10">
          <div className="flex flex-col gap-2">
            <p className="text-sm font-medium leading-relaxed">
              Recebemos de{" "}
              <strong className="uppercase">{receipt.payer.name}</strong>,
              {receipt.payer.company
                ? ` representante da empresa ${receipt.payer.company}, `
                : ""}
              {receipt.payer.cnpj
                ? `inscrita no CNPJ/CPF sob o nº ${receipt.payer.cnpj}, `
                : ""}
              a importância de{" "}
              <strong>{receipt.amountInWords.toUpperCase()}</strong>.
            </p>
        ...
        <div className="mt-8 flex justify-center py-8 relative z-10">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-xs font-black uppercase text-neutral-900 tracking-tighter">
              {receipt.receiver.name}
            </span>
            <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest">
              CNPJ: {receipt.receiver.cnpj}
            </span>
            <span className="text-[8px] font-medium text-neutral-400 uppercase tracking-widest italic mt-1">
