...
export default function InvoiceSuccessPage() {
  ...
  const [invoice, setInvoice] = React.useState<Record<string, any> | null>(
    null,
  );
  const [receiptData, setReceiptData] = React.useState<
    InvoiceReceiptPDFProps["data"] | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    axios
      .get(`/api/invoices/${invoiceId}/receipt${qs}`)
          ...
          <div className="p-10 flex flex-col gap-6 text-center">
            <div>
              ...
              <p className="text-neutral-500 font-bold uppercase tracking-widest text-[10px]">
                Sua fatura #{invoiceId.slice(-6).toUpperCase()} foi liquidada
                com sucesso.
              </p>
            </div>
                ...
                {({ loading }) => (
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        <Download className="w-4 h-4 mr-2" /> Baixar Recibo
                        (PDF)
                      </>
                    )}
                  </Button>
